...
Feature: MFA Challenge
  ...
  Scenario: MFA challenge should be created successfully
    ...
    Then the MFA attempts remaining should be 3
  ...
  Scenario: Wrong MFA code should decrement the failure counter
    ...
    Then the MFA attempts remaining should be 2
  ...
  Scenario: MFA challenge should be single-use
    ...
    And I verify the same MFA challenge again with code "123456"
    And the API response status should be 401
    Then the MFA error code should be "MFA_CHALLENGE_ALREADY_USED"