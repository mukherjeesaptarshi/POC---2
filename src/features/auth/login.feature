...
Feature: Authentication - Login and Token Issue
  ...
  Scenario: Login with valid credentials should return a valid access token
    ...
    When the API response status should be 200
    And the login response should contain a "access" token
    ...
    And the login response should not contain "invalid expiry date"
    And the login response should not contain "password"
    Then the login response should not contain "sensitive information"
    #And the login response should contain a valid expiry date
    #And the login response should not contain the password
    #Then the login response should not contain sensitive personal information
