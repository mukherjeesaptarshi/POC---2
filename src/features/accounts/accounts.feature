...
Feature: Banking account APIs
  ...
  Scenario: Get Accounts and Account by ID
    ...
    And the accounts list response matches the account list schema
    And the account detail response matches the account schema
    Then every returned account has numeric balances, ISO currency, and masked account number
  ...
  Scenario: Balance Reconciliation
    ...
    And the transaction history response matches the transaction list schema
    And the transaction history reconciles with the current account balance
    Then the account available, current, and pending balances are internally consistent
  ...
  Scenario: Transaction List - Filters and Paging
    ...
    And every filtered transaction satisfies the requested filter
    And paginated transaction pages have no duplicates
    Then the total transaction count remains consistent across pages
